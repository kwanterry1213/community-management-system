...
# 本地測試用還原腳本
# 假設伺服器在 localhost:5173 上運行
APP_URL = "http://localhost:5173"
# 測試環境密鑰，可與 app.py 中的 ADMIN_SECRET 相同
...
def restore_database(backup_file_path):
    ...
    try:
        ...
        if response.status_code == 200:
            print("[Success] 資料庫還原成功！")
        else:
            print(f"[Error] 還原失敗: {response.status_code} - {response.text}")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("使用方法: python restore_local.py <備份檔案路徑>")
        print("例如: python restore_local.py backups/backup_20260218_120000.db")
    else:
